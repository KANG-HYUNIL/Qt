...
int Chart_parent::magPointWidth = 7;
int Chart_parent::sizePointWidth = 9;
int Chart_parent::magPointWidth1_2 = magPointWidth/2;
int Chart_parent::sizePointWidth1_2 = sizePointWidth/2;
int Chart_parent::pointLineWidth = 2;
int Chart_parent::pointLineWidth1_2 = pointLineWidth/2;
int Chart_parent::chartIDCount = 0;

QPen Chart_parent::paintDrawPen  = QPen(QColor(150,80,80),pointLineWidth);
QBrush Chart_parent::paintFillPen  = QBrush(QColor(255,255,255));


//constructors
Chart_parent::Chart_parent(QWidget *parent, PaintChartType type, bool textType, bool mov, int mpc, int spc):QWidget(parent),magPoint(mpc),sizePoint(spc),movable(mov),chartText(textType)
{
    chartType = type;
    widgetPosInit();
    varInit();
...
Chart_parent::Chart_parent( int x, int y, int w, int h, QWidget *parent, PaintChartType type):QWidget(parent),magPoint(4),sizePoint(4)
{
    chartType = type;
...
Chart_parent::Chart_parent(Chart_parent &cb)
{
...
//init methods
void Chart_parent::widgetPosInit(int x,int y,int w,int h)
{
    widgetStart.setX(x);
    widgetStart.setY(y);
...
void Chart_parent::varInit(int mpw,int spw,int plw,bool sa,bool smo)
{
    qDebug()<<chartIDCount;
    ID = chartIDCount++;
    Chart_parent::magPointWidth = mpw;
    Chart_parent::sizePointWidth = spw;
    updatePaddingInfo();
    Chart_parent::pointLineWidth = plw;
    showAll = sa;
    showMag = smo;
    qDebug()<<"Create ";
}

void Chart_parent::paintInit()
{
    updatePaintInfo();
}

void Chart_parent::pointInit()
{
...
//update & adjus infos method
void Chart_parent::updatePaddingInfo()
{
...
void Chart_parent::updateWidgetPosInof()
{
#if 1
...
void Chart_parent::updatePaintInfo()
{
...
void Chart_parent::updateSizePointInfo()
{
...
void Chart_parent::updateMagPointInfo()
{
...
void Chart_parent::adjustPointInfo()
{
    for(auto it = magPoint.i_point.begin();it!=magPoint.i_point.end();it++)
    {
        switch((*it)->getRotate())
...
void Chart_parent::updateMagPointPath()
{
...
void Chart_parent::updateSizePointPath()
{
...
void Chart_parent::updateMagPointLine()
{
    for(auto it = magPoint.i_point.begin(),end = magPoint.i_point.end();it != end;it++)
    {
...
void Chart_parent::updateTextInfo()
{
...
void Chart_parent::paintMagPoint(QPainter & p)
{
    QPen tmp = p.pen();
...
void Chart_parent::paintSizePoint(QPainter & p)
{
    QPen tmp = p.pen();
    p.setPen(paintDrawPen);
...
void Chart_parent::paintSizeEdge(QPainter & p)
{
    QPen tmp = p.pen();
    p.setPen(paintDrawPen);
...
bool Chart_parent::inPath(const QPointF &p)
{
    for(auto it = magPoint.i_point.begin(),end = magPoint.i_point.end();it!=end;it++)
    {
...
bool Chart_parent::inMagPath(const QPointF &p, ORIENTION &b, int &index) const
{
    int i = 0;
...
bool Chart_parent::inSizePath(const QPointF &p, ORIENTION &b) const
{
    int x,y;
    for(auto it = sizePoint.i_point.begin(),end = sizePoint.i_point.end();it!=end;it++)
    {
...
void Chart_parent::setStartPos(int x,int y)
{
    widgetStart.setX(x);
    widgetStart.setY(y);
}
void Chart_parent::setEndPos(int x,int y)
{
    widgetEnd.setX(x);
    widgetEnd.setY(y);
}
...
void Chart_parent::setXY(int x, int y)
{
    int *x1,*y1;
    int *x2,*y2;
    if(widgetEnd.rx()<widgetStart.rx())
...
void Chart_parent::setWidthHeight(int x, int y)
{
#if 0
    ORIENTION type;
...
void Chart_parent::setWidthHeight(int x, int y, ORIENTION type)
{
    int *x1,*y1;
    int *x2,*y2;
    if(widgetEnd.rx()<widgetStart.rx())
...
void Chart_parent::applyWidthHeight()
{
    updateWidgetPosInof();
    updatePaintInfo();
    updateSizePointInfo();
...
bool Chart_parent::autoSetMagi(int &abcx,int &abcy, int &index)
{
    int mx = abcx;
    int my = abcy;
...
void Chart_parent::overlapChartMousePressed(QMouseEvent *event)
{
    int mx = event->pos().rx();
    int my = event->pos().ry();
    if(mx >= x() && my >= y() && mx-x()<=width() && my-y()<=width())
...
void Chart_parent::overlapChartMouseMove(QMouseEvent *event)
{
    int mx = event->pos().rx();
    int my = event->pos().ry();
...
void Chart_parent::deleteThisChart()
{
}


void Chart_parent::saveStaticValue(QDataStream &fout)
{
    fout.writeRawData(reinterpret_cast<const char*>(&Chart_parent::magPointWidth),sizeof(int));
    fout.writeRawData(reinterpret_cast<const char*>(&Chart_parent::sizePointWidth),sizeof(int));
    fout.writeRawData(reinterpret_cast<const char*>(&Chart_parent::pointLineWidth),sizeof(int));
    //fout.writeRawData(reinterpret_cast<const char*>(&Chart_parent::chartIDCount),sizeof(int));
    qDebug()<<magPointWidth<<" "<<sizePointWidth<<" "<<pointLineWidth<<" "<<chartIDCount;
}
void Chart_parent::loadStaticValue(QDataStream &fin)
{
    fin.readRawData(reinterpret_cast<char*>(&Chart_parent::magPointWidth),sizeof(int));
    fin.readRawData(reinterpret_cast<char*>(&Chart_parent::sizePointWidth),sizeof(int));
    fin.readRawData(reinterpret_cast<char*>(&Chart_parent::pointLineWidth),sizeof(int));
    updatePaddingInfo();
    //fin.readRawData(reinterpret_cast<char*>(&Chart_parent::chartIDCount),sizeof(int));
    qDebug()<<"static value:"<<magPointWidth<<" "<<sizePointWidth<<" "<<pointLineWidth<<" "<<chartIDCount;
}


QDataStream &operator<<(QDataStream &fout,  const Chart_parent &cb)
{
...
QDataStream &operator>>(QDataStream &fin, Chart_parent &cb)
{
...
void Chart_parent::paintEvent(QPaintEvent *event)
{
    QPainter p(this);
    paintChart(p);
    if(showAll)
...
void Chart_parent::mousePressEvent(QMouseEvent *event)
{
...
void Chart_parent::mouseMoveEvent(QMouseEvent *event)
{
    if(curFlag == MOUSE_EVENT_TYPE::NONE)
    {
        ORIENTION direct = ORIENTION::NONE;
...
void Chart_parent::mouseReleaseEvent(QMouseEvent *event)
{
    if(chartText.textType)
    {
...
void Chart_parent::mouseDoubleClickEvent(QMouseEvent *event)
{
    if(inPath(event->pos()))
    {
        emit sendThisClass(this,event->pos().rx()-borderWidth,event->pos().ry()-borderWidth);
...
void Chart_parent::leaveEvent(QEvent *event)
{
    hideMagOnly();
}
