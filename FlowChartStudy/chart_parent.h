#ifndef CHART_PARENT_H
#define CHART_PARENT_H
...
#define ENABLE 1    // 使能
#define DISABLE 0    // 失能
#define RUNTIME_PAINTMAG DISABLE     // 实时磁力点绘制
#define RUNTIME_PAINTSIZE DISABLE    // 实时大小点绘制


class Chart_parent : public QWidget
{
    Q_OBJECT
...
private:
    ...
    //reset all static variables value
    static void resetStaticVal(){
        Chart_parent::magPointWidth = 7;
        Chart_parent::sizePointWidth = 9;
        Chart_parent::magPointWidth1_2 = magPointWidth/2;
        Chart_parent::sizePointWidth1_2 = sizePointWidth/2;
        Chart_parent::pointLineWidth = 2;
        Chart_parent::pointLineWidth1_2 = pointLineWidth/2;
        Chart_parent::chartIDCount = 0;
    }
...
protected:
    ...
    {
    public:
        QPoint *i_pos = nullptr;        // point positions
    ...
    {
    public:
        std::vector<Chart_parent*> i_lineStart; // 每个磁力点连接的线的指针容器
        std::vector<Chart_parent*> i_lineEnd; // 每个磁力点连接的线的指针容器
...
public:
    //constructor
    explicit Chart_parent(QWidget *parent = nullptr, PaintChartType type = PaintChartType::NONE, bool textType = false, bool mov = true, int mpc=4, int spc=4);
    Chart_parent( int x, int y, int w, int h, QWidget *parent = nullptr, PaintChartType type = PaintChartType::NONE);
    explicit Chart_parent(Chart_parent &);
    virtual ~Chart_parent()
    {
        if(graphPath)
        {
            delete graphPath;
    ...
    void addMagiPointStartLine(int i,Chart_parent *cb){magPoint.i_point[i]->i_lineStart.push_back(cb);}   // 增加连线到相应的磁力点的起始连线容器
    void delMagiPointStartLine(int i,Chart_parent *cb)                                                    // 删除磁力点的起始连线容器中相应的连线
    {
    ...
    void addMagiPointEndLine(int i,Chart_parent *cb){magPoint.i_point[i]->i_lineEnd.push_back(cb);}       // 增加连线到相应的磁力点的终点连线容器
    void delMagiPointEndLine(int i,Chart_parent *cb)                                                      // 删除磁力点的终点连线容器中相应的连线
    {
    ...
    friend QDataStream &operator<<(QDataStream &fout, const  Chart_parent &cb);   // 输出运算符重载
    friend QDataStream &operator>>(QDataStream &fin, Chart_parent &cb);           // 输入运算符重载



signals:

    void sendThisClass(Chart_parent *,int x,int y);       // 发送自己给画布
    void setTypeChangeSize(ORIENTION i);                            // 设置画布中的鼠标状态为改变大小
    void setTypeCreateMagPoint(Chart_parent *,ORIENTION d, int i);    // 设置画布中的鼠标状态为创建连线
