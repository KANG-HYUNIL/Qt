...
class Chart_Line : public Chart_parent
{
...
#define OPTIMIZATION 0
    ...
    Chart_parent *startChartMag = nullptr;    // 起始图形
    Chart_parent *endChartMag = nullptr;      // 终止图形
...
public:
    ...
    Chart_Line(QWidget *parent = nullptr, PaintChartType type = PaintChartType::LINE) : Chart_parent(parent,type,true,false,0,2)
    {
    ...
    Chart_Line(Chart_Line &cr):Chart_parent(cr)
    {
    ...
    Chart_Line( int x1, int y1, int x2, int y2, QWidget *parent = nullptr,PaintChartType type = PaintChartType::LINE) : Chart_parent(x1,y1,x2,y2,parent,type)
    {
        chartText.textType1->setAttribute(Qt::WA_StyledBackground,true);
        chartText.textType1->setAutoFillBackground(true);
        chartText.textType1->setPalette(QPalette(QPalette::Window, QColor(255,255,255,150)));
    ...
    void setStartChart(Chart_parent* cb){startChartMag = cb;} // 设置起点指向的图形
    ...
    Chart_parent *getStartChart(){return startChartMag;}      // 获取起点指向的图形
    void setEndChart(Chart_parent* cb){endChartMag = cb;}     // 设置终点指向的图形
    ...
    Chart_parent *getEndChart(){return endChartMag;}          // 获取终点指向的图形
    ...
    friend QDataStream &operator>>(QDataStream &fin, Chart_Line &cl);
