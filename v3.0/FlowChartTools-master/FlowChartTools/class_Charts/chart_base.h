...
protected:
    ...
    public:
        ...
        //added codes
        i_pointbase(const i_pointbase& other) : rotate(other.rotate) {
            if (other.i_pos) i_pos = new QPoint(*other.i_pos);
            if (other.i_path) i_path = new QPainterPath(*other.i_path);
        }
    ...
    public:
        ...
        //added codes
        i_magpointbase(i_magpointbase& other) : i_pointbase(other)
        {
            i_lineStart.resize(other.i_lineStart.size());

            i_lineEnd.resize(other.i_lineEnd.size());
        ...
        ~i_magpointbase(){
            ...
            //added codes
            for (auto ptr : i_lineStart) {
                delete ptr;
            }
            for (auto ptr : i_lineEnd) {
                delete ptr;
            }
        }
    };
    ...
    public:
        ...
        //added codes
        i_magpoint(const i_magpoint& other) {
            i_point.resize(other.i_point.size());
            for (size_t i = 0; i < other.i_point.size(); ++i) {
                if (other.i_point[i]) {
                    i_point[i] = new i_magpointbase(*other.i_point[i]);
                } else {
                    i_point[i] = nullptr;
                }
            }
...
public:
    ...
    //added codes
    void copyData(Chart_Base* other);
