...
void FlowChart::insertRect(){
    int x = this->frameGeometry().width()/4;
    int y = this->frameGeometry().height()/3;
...
void FlowChart::insertText(){
    ...
    QPen DrawPen  = QPen(QColor(0,0,0,0),2);
    QBrush FillPen  = QBrush(QColor(255,255,255,0));

    emit sendChartStyle(DrawPen,FillPen);
    //curPaintChart->setWidthHeight(20,20,ORIENTION::SOUTHEAST);
    //curPaintChart->applyWidthHeight();
    curPaintChart->update();
    curPaintChart->show();
