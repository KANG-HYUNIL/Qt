    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(dialog,SIGNAL(create()),this,SLOT(newButtonClicked()));
    connect(dialog,SIGNAL(openExisting()),this,SLOT(openButtonClicked()));


    connect(ui->rectShoutcut,SIGNAL(clicked()),this,SLOT(on_rectShortcut_clicked()));
    connect(ui->textShortcut,SIGNAL(clicked()),ui->flowChart_widget,SLOT(on_textShortcut_clicked()));

    connect(ui->chart_capsule,SIGNAL(clicked()),ui->flowChart_widget,SLOT(setPaintChart_capsule()));
    connect(ui->chart_document,SIGNAL(clicked()),ui->flowChart_widget,SLOT(setPaintChart_document()));
    connect(ui->chart_parallelogram,SIGNAL(clicked()),ui->flowChart_widget,SLOT(setPaintChart_parallelogram()));
    connect(ui->chart_subroutine,SIGNAL(clicked()),ui->flowChart_widget,SLOT(setPaintChart_subroutine()));
    connect(ui->chart_circle,SIGNAL(clicked()),ui->flowChart_widget,SLOT(setPaintChart_circle()));
    ...
    connect(ui->openButton,SIGNAL(clicked()),ui->flowChart_widget,SLOT(openChartFile()));
    connect(ui->saveButton,SIGNAL(clicked()),ui->flowChart_widget,SLOT(saveChartFile()));
    connect(ui->newButton,SIGNAL(clicked()),ui->flowChart_widget,SLOT(newChartFile()));
    connect(ui->saveAs,SIGNAL(clicked()),ui->flowChart_widget,SLOT(saveasChartFile()));
    ...
    connect(ui->undoAction,SIGNAL(clicked()),ui->flowChart_widget,SLOT(undo()));
    connect(ui->redoAction,SIGNAL(clicked()),ui->flowChart_widget,SLOT(redo()));
...
{
    QString tmp = QString("流程图编辑器 - %1").arg(title);
    this->setWindowTitle(tmp);
}



void MainWindow::on_rectShortcut_clicked()
{
    ui->flowChart_widget->setPaintChart_Rect();
    ui->flowChart_widget->insertRect();
}



void MainWindow::on_textShortcut_clicked()
{
    ui->flowChart_widget->setPaintChart_Text();
    ui->flowChart_widget->insertText();
}
...
void MainWindow::openButtonClicked(){
    this->show();
    ui->flowChart_widget->openChartFile();
}
