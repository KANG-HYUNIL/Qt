...
//added codes
void Chart_Base::copyData(Chart_Base* other)
{
    if(!other)
    {
        return;
    ...
    this->showMag = other->showMag;
    this->lastType = other->lastType;
    this->curFlag = other->curFlag;
    this->curIndex = other->curIndex;
    this->chartType = other->chartType;
    this->ID = other->ID; //id는 복사 후 생성 시 설정
    this->movable = other->movable;

    this->magPointWidth = other->magPointWidth;
    this->sizePointWidth = other->sizePointWidth;
    this->magPointWidth1_2 = other->magPointWidth1_2;
    this->sizePointWidth1_2 = other->sizePointWidth1_2;
    this->pointLineWidth = other->pointLineWidth;
    this->pointLineWidth1_2 = other->pointLineWidth1_2;
    this->chartIDCount = other->chartIDCount;

    qDebug() << "copyData - variables";

    //Qpoints
    this->widgetStart = other->widgetStart;
    this->widgetEnd = other->widgetEnd;
    this->paintStart = other->paintStart;
    this->paintEnd = other->paintEnd;

    qDebug() << "copyData - QPoints";

    //Qpen, QBrush
    this->paintChartDrawPen = other->paintChartDrawPen;
    this->paintChartFillPen = other->paintChartFillPen;

    qDebug() << "copyData - QPen, QBrush";

    //graphPath
    if (other->graphPath)
    {
        if (this->graphPath)
            delete this->graphPath;
        this->graphPath = new QPainterPath(*other->graphPath);
    }
    else
    {
        if (this->graphPath)
        {
            delete this->graphPath;
            this->graphPath = nullptr;
        }
    }

    qDebug() << "copyData - graphPath";

    //이 아래, chartText 부분에서 처음으로 문제가 발생한다.

    //chartText
    this->chartText.textType = other->chartText.textType;
    this->chartText.chartTextMousePos = other->chartText.chartTextMousePos;

    if(this->chartText.textType2)
    {
        delete this->chartText.textType2;
        this->chartText.textType2 = nullptr;
    }


    if (other->chartText.textType2)
    {
        // if (this->chartText.textType2)
        //     delete this->chartText.textType2;
        this->chartText.textType2 = new Chart_PlainText();  // Create a new instance

        // Copy relevant data from other->chartText.textType2
        // For example:
        // this->chartText.textType2->setText(other->chartText.textType2->text());
        // You need to ensure Chart_PlainText has appropriate methods for copying state
    }
    else
    {
    ...
    qDebug() << "copyData - textType2";


    if (this->chartText.tmpEdit2)
    {
        delete this->chartText.tmpEdit2;
        this->chartText.tmpEdit2 = nullptr;
    }




    if (other->chartText.tmpEdit2)
    {
        ;
        this->chartText.tmpEdit2 = new QPlainTextEdit();

        // Copy relevant data from other->chartText.tmpEdit2
        // For example:
        // this->chartText.tmpEdit2->setPlainText(other->chartText.tmpEdit2->toPlainText());
        // You need to ensure QPlainTextEdit has appropriate methods for copying state
    }

    qDebug() << "copyData - tmpEdit2";

    this->sizePoint.i_point.resize(other->sizePoint.i_point.size());
    for (size_t i = 0; i < other->sizePoint.i_point.size(); ++i)
    {
        if (other->sizePoint.i_point[i])
        {
            this->sizePoint.i_point[i] = new i_pointbase(*other->sizePoint.i_point[i]);
        }
    }

    qDebug() << "copyData - sizePoint.i_point";


    this->magPoint.i_point.resize(other->magPoint.i_point.size());

    for (size_t i = 0; i < other->magPoint.i_point.size(); ++i)
    {
        if (other->magPoint.i_point[i])
        {
            this->magPoint.i_point[i] = new i_magpointbase(*other->magPoint.i_point[i]);

        }
    }

    qDebug() << "copyData - magPoint.i_point";


    textInit();

    //setPlainText has problem?
    chartText.textType2->setPlainText(other->chartText.textType2->toPlainText());

    qDebug() << other->chartText.textType2->toPlainText() + " : others";
    qDebug() << chartText.textType2->toPlainText() + " : this";

    disconnect(chartText.textType2, SIGNAL(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)), this, SLOT(setTypeChangeTextPos(CHART_TEXT_MOUSE_TYPE, int, int)));
